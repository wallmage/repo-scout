...
- The visible report answers six questions: what does it do, how does it work from start to finish, why does each major step improve the result, what concrete problem does it solve better than the obvious alternative, exactly what must be installed, and does it run on this machine. Keep it to one to two pages — at most three for a very large repository.
- Explain the mechanism in 10–15 numbered, plain-language steps. Simple targets get more concrete steps; complex targets group low-level actions into larger stages. Each step says what happens and why it helps.
- Translate internal labels into observable actions. Do not expect a newcomer to understand component names, artifact names, architecture labels, or feature counts; explain what the tool actually does and what difference the user experiences.
...
- Explain the chosen installation scope. When only one folder or component is recommended, say what useful capability lives there, what the excluded repository areas contain, and why those areas are not needed for this target.
